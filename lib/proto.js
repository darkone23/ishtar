...
function defprotocol(methods) {
  ...
  function genfn(protocol, key, index) {
    var fn = function(/* args */) {
      var args = Array.prototype.slice.call(arguments, 0);
      var name = getDispatchName(args[index]);
      try {
  ...
  if (valid(methods)) {
    ...
    iter(methods.keys(), function(key) {
      var index = methods.get(key)['args'].indexOf(self);
      protocol[key] = genfn(protocol, key, index);
    });
...
function getDispatchName(x) {
  if (x.constructor) {
    ...
    throw Error("Cannot dispatch against unknown object ", x);
  }
}
